...
import akka.stream.{ scaladsl, ActorMaterializer, ActorMaterializerSettings }
...
class SchemaRegistryDownloaderTest
    extends TestKit(ActorSystem("SchemeDownloaderTest"))
    with FlatSpecLike
    with Matchers
    with MockFactory
    with EncodeJsons {
  ...
  trait Downloader {
    ...
    val downloader = SchemaDownloader(
      Uri.Empty.withHost("www.schemaregistry.com").withScheme("http"),
      Seq("testschema"),
  ...
  "When searching subjects" should "create the next request with the ones found" in new Downloader
    with Schemas {
    val probe                  = TestProbe()
    ...
    scaladsl.Source
      ...
      .request(1)
      .expectNext(
        HttpRequest(uri = Uri.Empty.withPath(Path(s"/subjects/${subjects.head}/versions/latest")))
      )
      .onComplete()
  }

  it should "only complete without message in case it can't find the subject" in new Downloader
    with Schemas {
    val probe                  = TestProbe()
  ...
  "Saving files" should "" in new Downloader
    with Schemas {
      //    val probe = TestProbe()
